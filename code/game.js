var isdev = true;
if (isdev)
  alert("WARNING -- You are running the development test build")
...
scene('game', () => {
  ...
  onUpdate(() => {
    if (chickenShot && !stopped) {
      ...
      every('distanceText', (d) => { d.text = `${shortNum(distanceF)}ft travelled`; });
      distance = Math.floor(distanceF);

      altitude = ((height()-SCALE*1.1-chicken.pos.y) / SCALE);
      every('altitudeText', (a) => { a.text = `Altitude: ${shortNum(altitude)}ft`; });
      
      maxAltitude = Math.max(maxAltitude, altitude);
      every('maxAltitudeText', (m) => { m.text = `Top Altitude: ${shortNum(maxAltitude)}ft`; });

      every('velocityText', (v) => { 
        let val = (velocityMach >= 0.8 ? `Mach ${parseFloat(velocityMach).toFixed(2)}` : `${shortNum(velocity)} ft/s`);
        v.text = `Speed: ${val}`;
      });
  ...
  function showEndMenu() {
    ...
    if (!endMenuShown) {
      ...
      setTimeout(() => {
        ...
        add([
          text(`${shortNum(parseFloat(parseFloat(distanceF).toFixed(1)))}ft\n ${shortNum(parseFloat(parseFloat(maxAltitude).toFixed(1)))}ft\n ${shortNum(score)}\n +$${shortNum(moneyGain)}\n\n $${shortNum(STUFF.money)}`, {
            size: SCALE*0.27,
            align: 'right',
            width: SCALE*5,
            font: 'ubuntu',
